...
class Model_Neural_Network(nn.Module):
    ...
    def __init__(self, input_dim, lr=0.01, epochs=30, lambda_reg=0.00, weight_dec=0.0001):
        super().__init__()
...
def all_attributes():
    ...
    return bert_attributes + meta_attributes
...
if __name__ == "__main__":
    ...
    model.fit(X_tensor, y_tensor, batch_size=BATCH_SIZE)
    ...
    """
    background = X_tensor
    explain_data = X_tensor
    print(f"shape x tensor: {X_tensor.shape}")

    explainer = shap.GradientExplainer(model, background)
    shap_values = explainer.shap_values(explain_data)
    print(f"shape shap values: {shap_values.shape}")

    shap_vals_sample = shap_values[0].squeeze()
    input_vals_sample = explain_data[0].numpy()
    print(f"shap vals sample: {shap_vals_sample.shape}")
    print(f"input vals sample: {input_vals_sample.shape}")

    # get feature names
    feature_names = all_attributes()  # your function with 832 names

    # Create SHAP Explanation object
    explanation = shap.Explanation(
        values=shap_vals_sample,
        data=input_vals_sample,
